...
def save_peft_adapter(protocol, base_model_path: str, adapter_dir: str, logger_: logging.Logger) -> None:
    ...
    # Try to remap via the existing helper if available
    try:
        from src.core.key_remapping import remap_lora_keys
    ...
    # Strip the default adapter-name suffix (.default) so the keys match
    # the canonical PEFT-internal naming used by get_peft_model.
    canon_state = {}
    for k, v in lora_state.items():
        nk = k.replace(".lora_A.default.", ".lora_A.").replace(".lora_B.default.", ".lora_B.")
        canon_state[nk] = v
    missing, unexpected = peft_model.load_state_dict(canon_state, strict=False)
    if missing:
        logger_.warning("Missing LoRA keys (after canonical remap): %s ... (%d total)",
...
def _patch_trainer_for_max_steps(trainer, max_steps: int, logger_) -> None:
    ...
    def patched(epoch: int):
        ...
        for batch in loader:
            ...
            logger_.info("[max_train_steps] step=%d loss=%.4f step_time_ms=%.1f",
                         trainer.global_step, result.loss, result.step_time_ms)
            if trainer.global_step >= max_steps:
                logger_.info("[max_train_steps] reached %d, stopping", max_steps)
                break
        return {"train_loss": loss_sum / max(total, 1), "train_steps": total,
