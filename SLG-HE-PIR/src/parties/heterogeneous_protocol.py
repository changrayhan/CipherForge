...
class HeterogeneousProtocol:
    ...
    def step_train(self, batch: Dict, global_step: int) -> StepResult:
        ...
        # === S: GPU logits + CPU response ===
        # Gold token ids (output_ids) must be passed so S selects Enc(-V_gold)
        # instead of Enc(-V_argmax). This makes the S3PIR gradient = a_t - V_gold.
        gold_ids: Optional[torch.Tensor] = None
        if isinstance(batch, dict) and "output_ids" in batch:
            gold_ids = batch["output_ids"]
        ...
        u_priv = self.party_u.privselect_and_recover_dispatch({
            "s3pir_responses": s3pir_responses,
            "step": global_step,
        })
    ...
    def step_train_chunked(
        ...
        # S (one-shot; mmap zero-copy ~3 ms)
        # Gold token ids passed so S selects Enc(-V_gold) for the gradient.
        gold_ids: Optional[torch.Tensor] = None
        if isinstance(batch, dict) and "output_ids" in batch:
            gold_ids = batch["output_ids"]
        ...
        n_tokens = len(s3pir_responses)
        if n_tokens == 0:
            raise RuntimeError(f"step {global_step}: S returned 0 s3pir_responses")

        split = _split_into_chunks(n_tokens, chunk_tokens)
        n_chunks = split.n_chunks
        logger.info(
            "step_train_chunked: step=%d n_tokens=%d K=%d chunks of ~%d",
            global_step, n_tokens, n_chunks, chunk_tokens,
        )

        # U per chunk (add_mask)
        all_ct: List[bytes] = []
        if prof:
            prof.begin_phase("priv_U")
        for chunk_id, (c_start, c_end) in enumerate(zip(split.starts, split.ends)):
            ...
            ct_chunk = chunk_reply.get("ct_list") or []
            if len(ct_chunk) != (c_end - c_start):
                raise RuntimeError(
                    f"step {global_step} chunk {chunk_id}: U produced {len(ct_chunk)} cts "
