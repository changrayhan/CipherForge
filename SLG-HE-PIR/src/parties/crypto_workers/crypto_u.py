...
class CryptoUWorker:
    ...
    def handle_request(
        ...
        """Process a chunk of ``s3pir_responses`` and return masked ciphertexts.

        Args:
            payload: ``{"s3pir_responses": [...], "step": int}``.

        Returns:
            ``{"ct_list": List[bytes]}`` — one masked ciphertext per token,
            in the same order as the input responses.
        """
        ...
        s3pir_responses: List[Dict[str, Any]] = payload.get("s3pir_responses", [])
        step: int = int(payload.get("step", 0))

        ct_list: List[bytes] = [b""] * len(s3pir_responses)
        for i, resp in enumerate(s3pir_responses):
            t_flat = resp.get("t_flat", 0)
            parity_real = resp.get("parity_real_bytes", b"")
            parity_dummy = resp.get("parity_dummy_bytes", b"")
            permutation = int(resp.get("permutation", 0))
            selected = parity_real if permutation == 0 else parity_dummy
            if not selected:
                continue
