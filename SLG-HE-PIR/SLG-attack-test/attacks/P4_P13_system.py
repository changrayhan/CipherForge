...
class PSystemAttack(BaseAttack):
    ...
    def _get_relative_path(self, relative_path: str) -> Path:
        """获取相对于项目根目录的路径。"""
    ...
    def run(self) -> list[AttackVerdict]:
        ...
        # ── P-8: checkpoint 替换 ──────────────────────────────────────────
        checkpoint_dirs = [
            self._get_relative_path("baseline/classification_genrel/checkpoints"),
            self._get_relative_path("baseline/generation_ner/checkpoints"),
        ]
        ...
        # ── P-9: 日志结构泄露 ───────────────────────────────────────────
        log_dir = self._get_relative_path("baseline/classification_genrel/logs")
        if log_dir.exists():
        ...
        # ── P-11: dump_attacks 误启用 ──────────────────────────────────
        dumps_dir = self._get_relative_path("baseline/classification_genrel/dumps")
        ...
        # ── P-13: backup hints 确定性 ───────────────────────────────────
        hint_path = self._get_relative_path("bfv_cache/s3pir_hints/backup_hint_table.json")
        if hint_path.exists():
            try:
