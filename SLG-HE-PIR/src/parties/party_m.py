...
class PartyM:
    ...
    def backward_and_update(self, payload: Dict) -> Dict:
        ...
        for t_flat in range(n_tokens):
            ...
            # SEAL BatchDecoder returns values in [0, pm); centre to [-pm/2, +pm/2)
            # so that the signed s_share (already in [-pm/2, +pm/2)) adds in
            # the same domain and the PRG mask r_t cancels exactly.
        ...
        if expected_shape is not None:
            ...
            B, S = (H_M.shape[0], H_M.shape[1]) if H_M is not None else (1, 1)

        if B * S != n_tokens:
            logger.warning(
                "step %d: B*S=%d != n_tokens=%d — reshaping mismatch, using flat",
                step, B * S, n_tokens,
            )
            B, S = n_tokens, 1
