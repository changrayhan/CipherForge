...
class _LlamaUShard(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, position_ids=None):
        ...
        # Compute rotary embeddings manually
        # Shape: (1, seq_len, head_dim) for SDPA attention
        inv_freq = 1.0 / (self._rotary_base ** (torch.arange(0, self._rotary_ndims, 2, dtype=torch.float32, device=device) / self._rotary_ndims))
        freqs = torch.outer(position_ids[0].float(), inv_freq)  # (seq_len, head_dim//2)
        emb = torch.cat((freqs, freqs), dim=-1)  # (seq_len, head_dim)
        # Use bfloat16 to match hidden states dtype
        cos = emb.cos().to(hidden.dtype).unsqueeze(0)  # (1, seq_len, head_dim)
        sin = emb.sin().to(hidden.dtype).unsqueeze(0)  # (1, seq_len, head_dim)
...
class _LlamaMShard(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, position_ids=None):
        ...
        emb = torch.cat((freqs, freqs), dim=-1)  # (seq_len, head_dim)
        # Use bfloat16 to match hidden states dtype
        cos = emb.cos().to(hidden_states.dtype).unsqueeze(0)  # (1, seq_len, head_dim)
        sin = emb.sin().to(hidden_states.dtype).unsqueeze(0)  # (1, seq_len, head_dim)
        position_embeddings = (cos, sin)
    ...
    for layer in model.layers:
        ...
        if attn is not None:
            ...
            if hasattr(attn, "q_proj"):
                attn.q_proj = _LoRALinear(attn.q_proj, rank, alpha)
            if hasattr(attn, "k_proj"):
                attn.k_proj = _LoRALinear(attn.k_proj, rank, alpha)
            if hasattr(attn, "v_proj"):
                attn.v_proj = _LoRALinear(attn.v_proj, rank, alpha)
            if hasattr(attn, "o_proj"):
                attn.o_proj = _LoRALinear(attn.o_proj, rank, alpha)
        # MLP projections
        mlp = getattr(layer, "mlp", None)
        if mlp is not None:
            if hasattr(mlp, "gate_proj"):
                mlp.gate_proj = _LoRALinear(mlp.gate_proj, rank, alpha)
            if hasattr(mlp, "up_proj"):
                mlp.up_proj = _LoRALinear(mlp.up_proj, rank, alpha)
            if hasattr(mlp, "down_proj"):
                mlp.down_proj = _LoRALinear(mlp.down_proj, rank, alpha)
