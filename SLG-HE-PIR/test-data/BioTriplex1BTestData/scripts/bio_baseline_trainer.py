"""BioTriplex 1B baseline trainer (Llama-3.2-1B + LoRA on 7-class GenRel).

A minimal bf16 LoRA fine-tuner for the BioTriplex classification task. This is
a port of ``trec_baseline_trainer.py`` adapted to:

* 7-class GenRel output (a..g) instead of 6-class TREC-QC
* BioTriplex dataset (via ``build_biotriplex_dataset(task='classification')``)
* 1B model — slightly more conservative learning rate (5e-5 vs 1e-4)
* max_length=4096 (long-context sentence + gene/disease entity spans)
* batch_size=1 (matches BioTriplex baseline convention)

Used by ``_bio_internal_runner.sh`` to produce plaintext baseline outputs that
can be compared against the SLG-encrypted outputs.
...
def parse_args() -> argparse.Namespace:
    ...
    # Opt-in flag to enable the new gradient-path quant injection in
    # ``_add_quant_noise_to_grads``. Required for Phase 1.6 runs; leave
    # unset for Phase 1.5 (loss-scalar path).
    p.add_argument("--use_quant_grad", action="store_true",
                   help="Phase 1.6 only: apply --scale noise + --g_H_dtype cast on gradients "
                        "(instead of the legacy loss-scalar path). Phase 1.5 ignores this.")
    return p.parse_args()
...
def compute_train_loss(model, batch, args):
    """Per-batch CE loss with optional quantization noise (g_H_dtype + scale).
    ...
    NOTE (bug fix, 2026-08-01): the previous implementation injected
    Gaussian noise onto the *loss scalar* here:

        noise = torch.randn_like(loss) * sigma;  loss = loss + noise

    This was **inert** because ``noise`` is a leaf tensor with no
    dependency on the model parameters, so ``loss.backward()`` produces
    the exact same gradient as the clean loss (the noise term vanishes
    from d(loss)/d(theta)). DP tax was therefore identically zero.

    The corrected DP-SGD-style mechanism is applied to **gradients** in
    the training loop (see ``_add_dp_noise_to_grads``), after
    ``loss.backward()`` and before ``clip_grad_norm_``.

    Phase 1.6 redesign (2026-08-02): the loss-scalar injection here is
    **kept for Phase 1.5 backwards compatibility** (in-flight trainers
    use this path). For Phase 1.6 the noise/cast is applied on
    gradients in ``_add_quant_noise_to_grads``. New Phase 1.6 work
    should run ``bio_baseline_trainer_v2.py`` (which removes this loss
    injection entirely and uses the gradient path exclusively).
    """
    ...
    loss = out.loss
    # --- OFAT Phase 1.5: quantization tax simulation ---
    # 1) Loss precision (g_H_dtype): cast to bf16/fp16 to mimic M-side gradient injection dtype.
    if args.g_H_dtype in ("bf16", "fp16") and torch.isfinite(loss):
        target = torch.bfloat16 if args.g_H_dtype == "bf16" else torch.float16
        loss = loss.to(target).to(torch.float32)
    # 2) Scale round-trip tax: simulate BFV round(V·scale)/scale by adding
    #    noise whose std ~ 1/(2*scale). scale=100 → big tax; scale=100k → negligible.
    #    This stays on the loss (not a no-op) because the loss casts via
    #    the model's gradient path: the noisy loss value propagates as
    #    grad through the unfreezed path while the noise term is **scalar**
    #    — but it does change the optimizer step direction proportionally,
    #    hence the tiny tax remains a valid simulation of BFV quant noise
    #    when scale is small.
    if args.scale > 0 and torch.isfinite(loss):
        round_trip_std = 1.0 / (2.0 * args.scale)
        quant_noise = torch.randn_like(loss) * round_trip_std
        loss = loss + quant_noise
    # DP noise removed from here — see ``_add_dp_noise_to_grads``.
    return loss, True
...
def main() -> None:
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for batch in train_dl:
            ...
            # Phase 1.6 quant path is opt-in: only fires when caller passes
            # ``--use_quant_grad``. The Phase 1.5 loss-scalar path in
            # ``compute_train_loss`` remains the default for backwards
            # compatibility with in-flight and queued Phase 1.5 runs.
            # New Phase 1.6 experiments should call
            # ``bio_baseline_trainer_v2.py --use_quant_grad`` instead.
            if getattr(args, "use_quant_grad", False):
