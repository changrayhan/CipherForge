...
class DChiNoiseGenerator:
    ...
    def sample(self, eta: float) -> torch.Tensor:
        ...
        # We use ``torch.empty.exponential_(lambd, generator=...)`` because
        # ``torch.distributions.Gamma.rsample`` does not accept a generator
        # argument (reference doc §3.1).  ``exponential_(lambd)`` returns
        # ``Exp(rate=lambd)`` with mean ``1/lambd``.
        r = 0.0
        for _ in range(self.d):
            x = torch.empty(1, device=self.device, dtype=torch.float32)
...
class H15Privatizer:
    ...
    Args:
        config: dict with the following keys (all optional except where noted):
            ...
            * ``dp_alpha`` (float, default ``0.15``) — relative noise ratio.
    ...
    def __init__(self, config: Dict) -> None:
        ...
        self.alpha: float = float(self.config.get("dp_alpha", 0.15))
